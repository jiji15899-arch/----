-- CloudPress D1 스키마
-- Cloudflare D1 (SQLite) 전용 — Supabase PostgreSQL 대체
-- worker.ts 의 모든 쿼리와 1:1 매칭

-- ============================
-- profiles 테이블
-- worker.ts 사용 컬럼:
--   id, user_id, email, name, password_hash, role,
--   avatar_url, cf_api_key_encrypted, cf_email,
--   gh_token_encrypted, plan_id, created_at
-- ============================
...
CREATE TABLE IF NOT EXISTS plans (
  ...
  features    TEXT DEFAULT '[]'   -- JSON 배열을 TEXT로 저장
);
...
-- sites 테이블
-- worker.ts 사용 컬럼:
--   id, user_id, name, product_type, hosting_type, subdomain,
--   github_repo_url, cf_pages_url,
--   ec2_instance_id, ec2_public_ip, ec2_region,
--   wp_admin_url, vps_provider, status, plan,
...
-- domains 테이블
-- worker.ts 사용 컬럼:
--   id, user_id, domain, cf_zone_id,
--   nameserver_1, nameserver_2,
--   ns_status, ssl_status,
--   connected_site_id, created_at, verified_at
-- ============================
...
-- deployments 테이블
-- worker.ts 사용 컬럼:
--   id, site_id, status, log, triggered_at, completed_at
-- ============================
...
-- conversion_jobs 테이블
-- worker.ts 사용 컬럼:
--   id, site_id, user_id, status, input_type,
--   total_files, converted, stubs, warnings, created_at
-- ============================
...
--   tags, is_active, hosting_type, created_at, updated_at
-- ============================
CREATE TABLE IF NOT EXISTS products (
  ...
  icon         TEXT NOT NULL,
  tags         TEXT NOT NULL DEFAULT '[]',   -- JSON 배열을 TEXT로 저장
...
('AWS_REGION',            'ap-northeast-2'),   -- worker.ts 에서 sm.AWS_REGION 으로 참조
('EC2_DEFAULT_AMI',       ''),
('EC2_SECURITY_GROUP',    ''),
('EC2_KEY_PAIR',          ''),
('VULTR_API_KEY',         ''),
...
-- PayPal 웹훅에서 RETURNING user_id, plan_id 사용
-- ============================
...
CREATE TABLE IF NOT EXISTS audit_logs (
  id          TEXT PRIMARY KEY,
  admin_id    TEXT,                           -- 삭제된 사용자도 로그 보존 (NO FK)
